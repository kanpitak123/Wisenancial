...
## 9. Output-language guard and Risk D/E (branch `feat/ai-language-guard`, local commits, not pushed)

Trigger: with a Thai UI, a live Portfolio Review on `claude-fast` answered in Korean and a Risk Analysis answered in English. The backend already put `Output language: Thai` in the system prompt, but only there, and nothing checked the answer.

- **Instruction, three places.** Every AI prompt builder now states the language in the system prompt (`Respond only in Thai/English`, plus "never Korean/Chinese/Japanese"), restates it inside the user message (`languageInstruction`), and passes `expectedLanguage` to the manager. Covers chart insight, Trader/Investor review, risk, quiz, AI Picks, user and system news enrichment, the news classifier, and the legacy `/ai-advisor/analyze` (which had no language rule at all). The vendored guardrails pass is exempt: it owns its prompt and validates its own envelope.
- **Check + retry + refuse.** `ai-language.ts` judges the script of the prose fields: more than 10% Hangul/CJK/other script, or any single field mostly foreign, fails; Thai expected needs at least 30% Thai letters; English expected allows at most 5% Thai. Short values (enums, tickers, numbers) are ignored. A failure is retried once with a correction appended to the user message; a second failure throws `AI_WRONG_LANGUAGE` (502). Nothing is charged: only an accepted answer reaches the charge step, and discarded answers are logged as `FAILED` / 0 credits / `error_code=WRONG_LANGUAGE` with their real token counts, so the cost of our retries stays visible. News enrichment checks only `aiSummary` and `stockImpactAnalysis` (`aiTranslatedSummary` is Thai by design).
- **Cost note.** A retry is one extra provider call that we pay for and the user does not.
- **Risk Analysis D/E was empty because it was never fetched.** Yahoo returns `debtToEquity` only from `quoteSummary()` (one symbol per call), so it had been deferred and the frontend sent `null` (see the pending-item section in `docs/handoff/ai-prompt-audit.md`). The data exists: AAPL 78.4, MSFT 29.1, NVDA 17.0, PTT.BK 58.9 (percent). `MarketDataService.getRiskFundamentals` now also fetches it per symbol (concurrency 5, 12 h cache, failures not cached, one failure never drops P/E and beta), divides by 100 so it matches the prompt's `>1.0` rule, and `GET /stocks/fundamentals` and `AnalyticsPage` pass it through.

## 10. Portfolio Review numbers: labelled metrics + grounding check (branch `feat/ai-grounded-numbers`, local, not pushed)

Investigation of the qa@ Stock review ("MSFT return 932%", "total profit 2,874.76" vs "unrealized profit 4,154.76"), against the payload the backend sends (same data `GET /analytics/portfolio/17/overview` returns):

- **932% is real input, not a hallucination.** `MSFT: average_cost 50, current_price 516.17, unrealized_pnl_percent 932.34`. The QA seed bought MSFT at $50, so (516.17 − 50) / 50 = 932.34%. The model quoted the payload correctly; the seed data is what is unrealistic.
...
- Chart insight stays free and rule-based by default. The AI runs (5 credits) only when the request carries `useAi: true`, which the frontend sends only from a separate "AI analysis · 5 credits" button next to Refresh; a missing flag, `useAi: false`, or a stale client's `modelId` / `useRuleBased` all get the free insight. Nothing is charged without an explicit AI click. `POST /ai/news/enrich` is always billed: its old no-model branch ran the system-paid enrichment for any logged-in user.

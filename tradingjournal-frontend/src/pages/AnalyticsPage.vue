...
// ─── Main tab ─────────────────────────────────────────────────────────────────
const activeTab = ref<'dashboard' | 'growth' | 'performance' | 'winrate' | 'pnl'>('dashboard');
...
const loadAllData = async () => {
  ...
  await store.loadDashboard(filterStore.apiStartDate, filterStore.apiEndDate);

  // 2. โหลดข้อมูลปฏิทิน
  if (filterStore.apiEndDate) {
    const d = new Date(filterStore.apiEndDate);
    currentDate.value = new Date(d.getFullYear(), d.getMonth(), 1);
    await store.loadCalendarData(currentDate.value.getMonth(), currentDate.value.getFullYear());
  }
};
onMounted(async () => {
...
watch(
  ...
  async (newPort) => {
    if (newPort) {
      store.tabsLoaded = false;
      await loadAllData();
      if (activeTab.value !== 'dashboard') {
        await store.loadAnalysisTabs(filterStore.apiStartDate, filterStore.apiEndDate);
      }
    }
  },
);

// 🌟 2. ดักจับการเปลี่ยนวันที่ใน Header
watch(
...
watch(activeTab, async (tab) => {
  if (tab !== 'dashboard' && !store.tabsLoaded && portStore.activePortfolio) {
    await store.loadAnalysisTabs(filterStore.apiStartDate, filterStore.apiEndDate);
  }
});

// 🌟 ตัวจับการเปลี่ยนวันที่ใน Header คุมทั้งเว็บ
watch(
  () => [filterStore.apiStartDate, filterStore.apiEndDate],
  async () => {
    store.tabsLoaded = false; // สั่งรีเซ็ตให้โหลด Analysis Tabs ใหม่ถ้ากดสลับ
    await store.loadDashboard(filterStore.apiStartDate, filterStore.apiEndDate);
    ...
    // 🌟 Hybrid Calendar: วันที่เปลี่ยน ให้ปฏิทินวิ่งตามไปเดือนล่าสุดด้วย
    if (filterStore.apiEndDate) {
      const d = new Date(filterStore.apiEndDate);
      currentDate.value = new Date(d.getFullYear(), d.getMonth(), 1);
      await store.loadCalendarData(currentDate.value.getMonth(), currentDate.value.getFullYear());
...
const summaryStats = computed(() => [
  {
    label: 'Total Trades',
    value: store.monthlySummary.totalTrades,
    icon: 'tag',
  ...
  {
    label: 'Win Rate',
    value: `${store.monthlySummary.winRate}%`,
    icon: 'pie_chart',
  ...
  {
    label: 'Wins',
    value: store.monthlySummary.wins,
    icon: 'emoji_events',
  ...
  {
    label: 'Losses',
    value: store.monthlySummary.losses,
    icon: 'trending_down',
...
const calendarDays = computed(() => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    days.push({ day: i, dateStr, pnl: store.calendarPnL[dateStr] || 0 }); // 👈 ดึงข้อมูลตรงนี้
  }
  return days;
});
...
// ── Performance ───────────────────────────────────────────────────────────────
const currentPerfData = computed(() => store.performance?.[perfTab.value] ?? []);
...
const validPositions = computed(() =>
  (store.winRateByPosition || []).filter((d) => d.position === 'BUY' || d.position === 'SELL'),
);

// 🟢 2. เรียกใช้ validPositions แทน store.winRateByPosition
const wrPositionSeries = computed(() => winRateSeries(validPositions.value));
const wrPositionOpts = computed(() => winRateOpts(validPositions.value.map((d) => d.position)));

const wrDaySeries = computed(() => winRateSeries(store.winRateByDay || []));
const wrDayOpts = computed(() => winRateOpts((store.winRateByDay || []).map((d) => d.day)));
const wrMonthSeries = computed(() => winRateSeries(store.winRateByMonth || []));
const wrMonthOpts = computed(() => winRateOpts((store.winRateByMonth || []).map((d) => d.month)));
const wrTimeSeries = computed(() => winRateSeries(store.winRateByTimeSlot || []));

// 🟢 3. เพิ่มการเอียงตัวหนังสือ -45 องศาให้ Time of Day (00:00-00:30 จะได้ไม่ซ้อนทับกัน)
const wrTimeOpts = computed(() => {
  const opts = winRateOpts((store.winRateByTimeSlot || []).map((d) => d.slot));
...
// ── AI Insight helpers ────────────────────────────────────────────────────────
const aiInsight = (key: string) => store.aiInsights[key] ?? '';
const aiLoading = (key: string) => store.aiLoading[key] ?? false;
...
// ── Refresh handlers ──────────────────────────────────────────────────────────
const refreshGrowth = () => {
  if (store.monthlyGrowth.length) void store.analyze('monthly_growth', store.monthlyGrowth);
};

const refreshPerf = () => {
  if (!store.performance || !perfTab.value) return;
  ...
  const data = store.performance[perfTab.value];
  if (data?.length) void store.analyze(key, data);
};

const refreshWinRate = () => {
  const map: Record<string, object> = {
    position: store.winRateByPosition,
    day: store.winRateByDay,
    month: store.winRateByMonth,
    time: store.winRateByTimeSlot,
  };
  const key = `winrate_${winTab.value}`;
  const data = map[winTab.value];
  if (data) void store.analyze(key, data);
};

const refreshPnl = () => {
  const map: Record<string, object> = {
    time: store.pnlByTimeSlot,
    day: store.pnlByDay,
    month: store.pnlByMonth,
  };
...
const pnlTimeSeries = computed(() => [
  {
    name: 'PnL ($)',
    data: store.pnlByTimeSlot.map((d) => d.total_pnl),
  },
]);
const pnlTimeOpts = computed<ApexOptions>(() =>
  barOpts({
    colors: store.pnlByTimeSlot.map((d) => (d.total_pnl >= 0 ? cc.value.green : cc.value.red)),
    xaxis: {
      categories: store.pnlByTimeSlot.map((d) => d.slot),
...
const pnlDaySeries = computed(() => pnlGroupedSeries(store.pnlByDay));
const pnlDayOpts = computed(() =>
  barOpts({
    colors: [cc.value.green, cc.value.red],
    xaxis: {
      categories: store.pnlByDay.map((d) => d.day),
      labels: { style: { colors: cc.value.text, fontSize: '11px' } },
      axisBorder: { show: false },
      axisTicks: { show: false },
    },
...
const pnlMonthSeries = computed(() => pnlGroupedSeries(store.pnlByMonth));
const pnlMonthOpts = computed(() =>
  barOpts({
    colors: [cc.value.green, cc.value.red],
    xaxis: {
      categories: store.pnlByMonth.map((d) => d.month),
      labels: { style: { colors: cc.value.text, fontSize: '11px' } },
      axisBorder: { show: false },
      axisTicks: { show: false },
    },
  ...
  <q-page class="analytics-page q-pa-md q-pa-sm-md">
    ...
    <div class="q-mb-md">
      ...
      >
        ...
        <q-tab name="growth" icon="bar_chart" label="Monthly Growth" />
        <q-tab name="performance" icon="insights" label="Performance" />
        <q-tab name="winrate" icon="emoji_events" label="Win Rate" />
        <q-tab name="pnl" icon="attach_money" label="PnL Charts" />
      </q-tabs>
    </div>

    <!-- loading spinner for analysis tabs -->
    <div
      v-if="store.loadingTabs && activeTab !== 'dashboard'"
    ...
    <template v-else>
        ...
        <q-card class="dashboard-card q-pa-md q-mb-md">
          <div class="row items-center justify-between q-mb-md header-divider q-pb-sm">
            ...
            <q-btn-toggle
              ...
              size="12px"
              @update:model-value="store.setChartFilter($event)"
            />
          </div>
          <VueApexCharts
            type="area"
      ...
      <!-- TAB: MONTHLY GROWTH                                                 -->
      <!-- ════════════════════════════════════════════════════════════════════ -->
      <div v-if="activeTab === 'growth' && !store.loadingTabs">
        <div class="row q-col-gutter-md q-mb-md">
          <!-- Chart -->
      ...
      <!-- TAB: PERFORMANCE                                                    -->
      <!-- ════════════════════════════════════════════════════════════════════ -->
      <div v-if="activeTab === 'performance' && !store.loadingTabs">
      ...
      <!-- TAB: WIN RATE                                                       -->
      <!-- ════════════════════════════════════════════════════════════════════ -->
      <div v-if="activeTab === 'winrate' && !store.loadingTabs">
        <div class="dashboard-card q-pa-sm q-mb-md">
          <q-tabs
            v-model="winTab"
            dense
            ...
            <q-card class="dashboard-card q-pa-md">
              ...
              <template v-else>
                <div style="overflow-x: auto">
                  <div :style="{ minWidth: store.winRateByTimeSlot.length * 40 + 'px' }">
                    <VueApexCharts
                      type="bar"
                      height="260"
                      :options="wrTimeOpts"
      ...
      <!-- TAB: PNL CHARTS                                                     -->
      <!-- ════════════════════════════════════════════════════════════════════ -->
      <div v-if="activeTab === 'pnl' && !store.loadingTabs">
        <div class="dashboard-card q-pa-sm q-mb-md">
          <q-tabs
            v-model="pnlTab"
            dense
          ...
          <div class="col-12 col-md-8">
            <q-card class="dashboard-card q-pa-md">
              ...
              <template v-if="pnlTab === 'time'">
                <div style="overflow-x: auto">
                  <div :style="{ minWidth: store.pnlByTimeSlot.length * 40 + 'px' }">
                    <VueApexCharts
                      type="bar"
                      height="260"
                      :options="pnlTimeOpts"
        ...
        <div class="row q-col-gutter-sm">
          <template v-if="pnlTab === 'time'">
            <div
              v-for="row in store.pnlByTimeSlot.filter((d) => d.total_pnl !== 0)"
              :key="row.slot"
          ...
          <template v-else-if="pnlTab === 'day'">
            <div v-for="row in store.pnlByDay" :key="row.day" class="col-6 col-sm-4 col-md-3">
              <q-card class="dashboard-card q-pa-sm">
          ...
          <template v-else>
            <div v-for="row in store.pnlByMonth" :key="row.month" class="col-6 col-sm-4 col-md-3">
              <q-card class="dashboard-card q-pa-sm">

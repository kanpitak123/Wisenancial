...
import GlobalDateFilter from 'components/GlobalDateFilter.vue'; // 👈 นำเข้า Component ปุ่มเลือกวันที่
...
onMounted(() => {
  void communityStore.fetchPosts();
});
...
const filteredPosts = computed(() => {
  ...
  if (selectedAssetFilter.value) {
    posts = posts.filter((p) => p.asset_name === selectedAssetFilter.value);
  }
        ...
        <q-card v-for="post in filteredPosts" :key="post.id" class="post-card q-mb-md">
          <q-card-section>
            ...
            <div v-if="post.post_images && post.post_images.length > 0" class="q-mt-md">
              <q-img
                :src="`http://localhost:3000${post.post_images[0]?.image_url}`"

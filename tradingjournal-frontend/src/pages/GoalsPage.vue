...
onMounted(() => {
  if (portStore.portfolios.length === 0) {
    void portStore.loadPortfolios();
  }
  const activePort = portStore.activePortfolio;
  if (activePort) {
    void store.loadGoalByMonth(activePort.id, new Date().getFullYear(), new Date().getMonth() + 1);
  }
});
...
const isFutureMonth = computed(() => {
  ...
  return (
    store.monthlyPlan.currentYear > today.getFullYear() ||
    (store.monthlyPlan.currentYear === today.getFullYear() &&
      store.monthlyPlan.currentMonth >= today.getMonth() + 1)
  );
});
...
const changeMonth = async (offset: number) => {
  if (!portStore.activePortId) {
    $q.notify({ type: 'warning', message: 'Please select a portfolio first', position: 'top' });
    return;
  }
  let newMonth = store.monthlyPlan.currentMonth + offset;
  let newYear = store.monthlyPlan.currentYear;
  if (newMonth < 1) {
    newMonth = 12;
    newYear -= 1;
  } else if (newMonth > 12) {
  ...
  await store.loadGoalByMonth(portStore.activePortId, newYear, newMonth);
};
...
const saveGoal = () => {
  ...
  if (inputTarget.value > 0) {
    void store.setTargetPnL(
      portStore.activePortId,
      store.monthlyPlan.currentYear,
      store.monthlyPlan.currentMonth,
      inputTarget.value,
    );
          ...
          <div class="row items-center q-gutter-sm">
            ...
            <div>
              <div class="text-subtitle1 text-weight-bolder text-main">
                {{ store.monthlyPlan.monthName }} {{ store.monthlyPlan.currentYear }}
              </div>
      ...
      <q-card class="goal-card q-pa-md">
        ...
        <div class="day-grid">
          ...
          >
            ...
            >
              <span v-if="day.status === 'success'"> +{{ fmtMoney(day.shortfall) }} surplus </span>
              <span v-else> -{{ fmtMoney(Math.abs(day.shortfall)) }} short </span>
            </div>
          </div>
        </div>
      </q-card>

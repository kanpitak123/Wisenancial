...
const selectedImpact = ref(impactOptions[0]);
...
const expandedRow = ref<number | null>(null); // เก็บ ID ข่าวที่กำลังกดอ่านรายละเอียด
...
const applyFilters = async () => {
  ...
  await newsStore.setFilters(selectedCurrency.value.value, selectedImpact.value.value);
};

// 🖱️ สลับเปิด-ปิด อ่านรายละเอียดข่าว
const toggleDetails = (newsId: number) => {
  expandedRow.value = expandedRow.value === newsId ? null : newsId;
};

// 📌 ปักหมุด
const handlePin = async (newsId: number) => {
  await newsStore.togglePin(newsId);
};
        ...
        >
          <q-card-section class="q-pa-md row items-center no-wrap">
            <div class="col-3 col-sm-2 column q-pr-sm">
              ...
              <span class="text-caption text-grey-6">{{ formatDate(news.date) }}</span>
            </div>

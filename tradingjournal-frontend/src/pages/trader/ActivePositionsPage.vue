...
// ==========================================
// 🚀 Positions = JournalStore.activeTrades + live quotes from MarketStore
// ==========================================
interface PositionRow extends Trade {
...
const positions = computed<PositionRow[]>(() =>
  journalStore.activeTrades.map((trade) => {
    const currentPrice = marketStore.quotes[trade.pair] ?? null;
    const openPrice = Number(trade.open_price ?? 0);
    const effectivePrice = currentPrice ?? openPrice;
    const diff =
      trade.trade_type === 'BUY' ? effectivePrice - openPrice : openPrice - effectivePrice;
...
const buyPositions = computed(() => positions.value.filter((p) => p.trade_type === 'BUY'));
const sellPositions = computed(() => positions.value.filter((p) => p.trade_type === 'SELL'));
...
const loadPositions = async () => {
  ...
  await safeLoad(() => journalStore.loadPortfolio(portfolioId), 'โหลดสถานะที่เปิดอยู่ไม่สำเร็จ');
};
...
// ดึงราคาปัจจุบันของคู่เงินที่มีสถานะเปิดอยู่
watch(
  () => journalStore.activeTrades.map((trade) => trade.pair),
  (pairs) => {
    const unique = [...new Set(pairs)].filter(Boolean);
    if (unique.length > 0) {
      void safeLoad(() => marketStore.fetchQuotes(unique), 'ดึงราคาปัจจุบันไม่สำเร็จ');
    }
  },
  { immediate: true },
);
...
const submitAddPosition = async () => {
  ...
  try {
    await journalStore.createOpenTrade({
      pair: formData.value.asset,
      trade_type: formData.value.trade_type,
      ...(formData.value.volume !== null ? { volume: formData.value.volume } : {}),
      ...(formData.value.entry_price !== null ? { open_price: formData.value.entry_price } : {}),
      ...(formData.value.entry_date
        ? { opened_at: new Date(formData.value.entry_date).toISOString() }
        : {}),
      ...(formData.value.strategy ? { strategy: formData.value.strategy } : {}),
    });
    ...
    $q.notify({
      type: 'negative',
      message: journalStore.error || 'Failed to open trade.',
      position: 'top',
    });
  } finally {
...
const tradeToClose = ref<PositionRow | null>(null);
const closeForm = ref({
...
const submitClosePosition = async () => {
  ...
  try {
    await journalStore.closeTrade(tradeToClose.value.id, {
      close_price: Number(closeForm.value.close_price),
      closed_at: new Date(closeForm.value.close_date).toISOString(),
      ...(closeForm.value.note ? { note: closeForm.value.note } : {}),
    });
    ...
    $q.notify({
      type: 'negative',
      message: journalStore.error || 'Failed to close trade.',
      position: 'top',
    });
  }
};
    ...
    <div class="row q-col-gutter-md q-mb-xl">
      <div
        ...
        v-for="(val, label, index) in {
          'Active Trades': totalPositions,
          'Buy Volume': buyPositions.length,
          'Sell Volume': sellPositions.length,
        }"
        :key="label"
      >
          ...
          <q-tr :props="props" :class="{ 'closed-row': props.row._closed }">
            ...
            <q-td key="entry" :props="props">
              ...
              <div class="text-caption text-muted">
                {{ formatTimeShort(props.row.opened_at) }}
              </div>
            </q-td>

...
describe('UserStore settings actions', () => {
  ...
  it('changePassword พัง -> flag กลับเป็น false, เก็บข้อความจากหลังบ้าน และโยน error ต่อ', async () => {
    ...
    await expect(store.changePassword({ current_password: 'x', new_password: 'y' })).rejects.toThrow();
  ...
  it('exportMyData คืนก้อนข้อมูลและปิด flag', async () => {
    const bundle = { format_version: 1, exported_at: '2026-09-26T00:00:00.000Z', user: {}, data: {} };
  ...
  it('clear() ล้าง flag ใหม่ทั้งสองตัวด้วย', () => {
    const store = useUserStore();

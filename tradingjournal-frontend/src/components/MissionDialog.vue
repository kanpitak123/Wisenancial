<!-- <template>
    <q-dialog v-model="isOpen" backdrop-filter="blur(8px) saturate(1.3)">
      <q-card class="mission-dialog-card" style="width: 500px; max-width: 95vw">
        ...
        <q-scroll-area style="height: 60vh; min-height: 400px">
          <div v-if="missionStore.loading" class="flex flex-center q-py-xl">
          ...
          <div v-else class="q-pa-md">
            <div v-for="m in missionStore.missions" :key="m.id" class="mission-card q-mb-md">
              ...
              <div class="q-mt-md border-top q-pt-sm flex justify-end">
                ...
                />
                <q-btn
                  v-else-if="m.progress >= m.target_count"
                  label="Claim Reward"
                  color="positive"
                  unelevated
                  dense
  ...
  async function handleClaim(missionId: number) {
    ...
    try {
      ...
      $q.notify({
        type: 'negative',
        message: e.message || 'Failed to claim reward.',
        position: 'top',
      });
    } finally {
      $q.loading.hide();
  ...
  /* Dark Mode Alignment (Matching MainLayout Layout) */
  .body--dark .mission-dialog-card {
    /* Match the core dark background used in MainLayout components */
    background: #151e32 !important;
  }

  .body--dark .mission-card {
    /* Softer tone inside the dialog card for better visibility */
    background: #151e32 !important;
    border-color: #23314b !important;
  }

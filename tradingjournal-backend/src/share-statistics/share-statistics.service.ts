...
export class ShareStatisticsService {
  ...
  async getShareStatistics(userId: number, portfolioId: number) {
    ...
    return portfolio.portfolio_type === PortfolioType.TRADER
      ? this.getTraderStatistics(portfolio)
      : this.getInvestorStatistics(portfolio);
  }
  ...
  async generateShareMessage(
    ...
    const message =
      stats.portfolio_type === PortfolioType.TRADER
        ? this.buildTraderMessage(stats, platform)
        : this.buildInvestorMessage(stats, platform);
  ...
  async generateShareImage(userId: number, portfolioId: number) {
    const stats = await this.getShareStatistics(userId, portfolioId);
    ...
    return {
      ...
      stats_for_image: {
        title: stats.portfolio_info.name,
        portfolio_type: stats.portfolio_type,
        currency: stats.portfolio_info.currency,
        return: `${stats.performance_summary.total_return}%`,
        pnl: stats.performance_summary.total_pnl,
        primary_metric:
          stats.portfolio_type === PortfolioType.TRADER
            ? {
                label: 'Win Rate',
                value: `${stats.performance_summary.win_rate}%`,
              }
            : {
                label: 'Holdings',
                value: stats.performance_summary.open_holdings,
              },
      },
    };
  }
  ...
  async logShareActivity(
    ...
    const log = await this.prisma.share_logs.create({
      data: {
        user_id: userId,
        ...
        stats_snapshot: this.toJson(stats),
      },
    });
  ...
  async getShareLogs(userId: number, portfolioId: number, limit = 20) {
  ...
  async getSocialSharingData(userId: number, portfolioId: number) {
    const stats = await this.getShareStatistics(userId, portfolioId);

    const [twitter, facebook, linkedin] = await Promise.all([
      this.generateShareMessage(userId, portfolioId, 'twitter'),
      this.generateShareMessage(userId, portfolioId, 'facebook'),
      this.generateShareMessage(userId, portfolioId, 'linkedin'),
    ]);
    ...
    const publicUrl = `${appUrl}/share/portfolio/${portfolioId}`;
    ...
    return {
      ...
      share_urls: {
        ...
        facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
          publicUrl,
        )}&quote=${encodeURIComponent(facebook.message)}`,
  ...
  private async getTraderStatistics(portfolio: PortfolioRow) {
    const trades = await this.prisma.trades.findMany({
      ...
      orderBy: [{ closed_at: 'asc' }, { created_at: 'asc' }],
      select: {
        pnl: true,
        result_status: true,
        closed_at: true,
    ...
    const totalProfit = this.sum(positive);
    const totalLoss = Math.abs(this.sum(negative));
    const totalPnl = this.sum(pnlValues);

    const firstDate = this.firstValidDate(
      trades.map((trade) => trade.closed_at ?? trade.created_at),
    );
    const lastDate = this.lastValidDate(
      trades.map((trade) => trade.closed_at ?? trade.created_at),
    );

    return {
      portfolio_type: PortfolioType.TRADER,
      portfolio_info: this.portfolioInfo(portfolio),
      performance_summary: {
        total_trades: trades.length,
        winning_trades: positive.length,
        losing_trades: negative.length,
        win_rate: this.percent(positive.length, trades.length),
        total_return: this.totalReturn(portfolio),
        total_pnl: this.round(totalPnl),
        active_days: this.daysBetween(firstDate, lastDate),
        open_holdings: 0,
      },
      profitability_metrics: {
        max_profit: this.round(positive.length ? Math.max(...positive) : 0),
        max_loss: this.round(
          negative.length ? Math.abs(Math.min(...negative)) : 0,
        ),
        average_profit: this.average(positive),
        average_loss: this.round(Math.abs(this.average(negative))),
        profit_factor:
          totalLoss > 0
            ? this.round(totalProfit / totalLoss)
            : totalProfit > 0
              ? null
              : 0,
      },
      risk_metrics: this.calculateRiskMetrics(
        pnlValues,
        Number(portfolio.initial_balance),
      ),
      monthly_performance: this.groupMonthly(
        trades.map((trade) => ({
  ...
  private async getInvestorStatistics(portfolio: PortfolioRow) {
    const [purchases, sales, dividends, prices] = await Promise.all([
    ...
    const priceMap = new Map(
      prices.map((price) => [price.symbol, Number(price.price)]),
    );
    ...
    for (const purchase of purchases) {
      const remaining = Number(purchase.remaining_shares);

      if (remaining <= 0) {
        continue;
      }

      const symbol = purchase.stock_symbol;
      const costPerShare = Number(purchase.purchase_price);
      const currentPrice = priceMap.get(symbol) ?? costPerShare;
      ...
      existing.total_cost += remaining * costPerShare;
      existing.market_value += remaining * currentPrice;
      existing.unrealized_pnl += remaining * (currentPrice - costPerShare);
    ...
    const holdings = [...holdingsMap.values()].map((holding) => ({
      ...holding,
      shares: this.round(holding.shares),
      total_cost: this.round(holding.total_cost),
      market_value: this.round(holding.market_value),
      unrealized_pnl: this.round(holding.unrealized_pnl),
      average_cost:
        holding.shares > 0
          ? this.round(holding.total_cost / holding.shares)
          : 0,
    }));

    const realizedPnl = this.sum(
      sales.map((sale) => Number(sale.realized_pnl)),
    );
    const dividendIncome = this.sum(
      dividends.map((dividend) => Number(dividend.net_amount)),
    );
    const unrealizedPnl = this.sum(
      holdings.map((holding) => holding.unrealized_pnl),
    );

    const totalPnl = realizedPnl + dividendIncome + unrealizedPnl;

    const initialBalance = Number(portfolio.initial_balance);
    const investorTotalReturn =
      initialBalance > 0 ? this.round((totalPnl / initialBalance) * 100) : 0;

    const firstDate = this.firstValidDate([
      ...purchases.map((item) => item.purchase_date),
      ...sales.map((item) => item.sold_date),
    ]);
    const lastDate = this.lastValidDate([
      ...purchases.map((item) => item.purchase_date),
      ...sales.map((item) => item.sold_date),
      ...dividends.map((item) => item.payment_date),
    ]);
    ...
    return {
      ...
      portfolio_info: this.portfolioInfo(portfolio),
      performance_summary: {
        total_trades: sales.length,
        winning_trades: sales.filter((sale) => Number(sale.realized_pnl) > 0)
          .length,
        losing_trades: sales.filter((sale) => Number(sale.realized_pnl) < 0)
          .length,
        win_rate: this.percent(
          sales.filter((sale) => Number(sale.realized_pnl) > 0).length,
          sales.length,
        ),
        total_return: investorTotalReturn,
        total_pnl: this.round(totalPnl),
        realized_pnl: this.round(realizedPnl),
        unrealized_pnl: this.round(unrealizedPnl),
        dividend_income: this.round(dividendIncome),
        active_days: this.daysBetween(firstDate, lastDate),
        open_holdings: holdings.length,
      },
      profitability_metrics: {
        best_sale: this.round(
          Math.max(0, ...sales.map((sale) => Number(sale.realized_pnl))),
        ),
        worst_sale: this.round(
          Math.min(0, ...sales.map((sale) => Number(sale.realized_pnl))),
        ),
        average_realized_pnl: this.average(
          sales.map((sale) => Number(sale.realized_pnl)),
        ),
      },
      risk_metrics: this.calculateRiskMetrics(
      ...
      monthly_performance: this.groupMonthly([
        ...sales.map((sale) => ({
          date: sale.sold_date,
          value: Number(sale.realized_pnl),
        })),
        ...dividends.map((dividend) => ({
          date: dividend.payment_date,
          value: Number(dividend.net_amount),
        })),
      ]),
  ...
  private buildTraderMessage(stats: any, platform: SharePlatform) {
    const summary = stats.performance_summary;
  ...
  private buildInvestorMessage(stats: any, platform: SharePlatform) {
    const summary = stats.performance_summary;
  ...
  private renderShareCardSvg(stats: any): string {
    const portfolioName = this.escapeXml(stats.portfolio_info.name);
    const portfolioType = stats.portfolio_type;
    const currency = this.escapeXml(stats.portfolio_info.currency);
    const summary = stats.performance_summary;
    const returnValue = Number(summary.total_return ?? 0);
    const pnlValue = Number(summary.total_pnl ?? 0);
    const positive = returnValue >= 0;
    const accent = positive ? '#17A673' : '#D84A4A';

    const primaryLabel =
      portfolioType === PortfolioType.TRADER ? 'WIN RATE' : 'HOLDINGS';
    const primaryValue =
      portfolioType === PortfolioType.TRADER
        ? `${summary.win_rate}%`
        : String(summary.open_holdings);
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  ...
  private async requirePortfolio(userId: number, portfolioId: number) {
    if (!Number.isInteger(portfolioId) || portfolioId <= 0) {
      throw new BadRequestException('portfolioId ไม่ถูกต้อง');
    }
  ...
  private totalReturn(portfolio: {
    ...
    return initial > 0 ? this.round(((current - initial) / initial) * 100) : 0;
  }
  ...
  private calculateRiskMetrics(
    ...
    const average = this.sum(pnlValues) / pnlValues.length;
    const variance =
      this.sum(pnlValues.map((value) => Math.pow(value - average, 2))) /
      pnlValues.length;
    ...
    return {
      sharpe_ratio: volatility > 0 ? this.round(average / volatility) : 0,
  ...
  private groupMonthly(
    ...
    for (const row of rows) {
      ...
      monthly.set(key, (monthly.get(key) ?? 0) + row.value);
    }
  ...
  private firstValidDate(dates: Array<Date | null>) {
    return (
      dates
  ...
  private lastValidDate(dates: Array<Date | null>) {
    const valid = dates
      .filter((date): date is Date => date instanceof Date)
  ...
  private daysBetween(from: Date | null, to: Date | null) {
    if (!from || !to) {
      return 0;
    }
  ...
  private percent(numerator: number, denominator: number) {
    return denominator > 0 ? this.round((numerator / denominator) * 100) : 0;
  }

  private average(values: number[]) {
    return values.length > 0 ? this.round(this.sum(values) / values.length) : 0;
  }

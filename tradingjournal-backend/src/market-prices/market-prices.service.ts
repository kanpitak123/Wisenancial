...
export class MarketPricesService {
  ...
  upsert(dto: UpsertMarketPriceDto) {
    ...
    const priceDate = dto.price_date ? new Date(dto.price_date) : new Date();

    if (Number.isNaN(priceDate.getTime())) {
      throw new BadRequestException('price_date is invalid');
    }
  ...
  find(symbols?: string[], currency?: string) {
    const normalizedSymbols = symbols?.length
      ? [...new Set(symbols.map((symbol) => this.normalizeSymbol(symbol)))]
      : undefined;
    ...
    return this.prisma.market_prices.findMany({
      ...
      orderBy: [{ symbol: 'asc' }, { currency: 'asc' }],
    });
  }

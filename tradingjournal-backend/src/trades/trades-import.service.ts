      ...
      async (tx) => {
        ...
        for (const row of rows) {
          ...
          await this.tradesService.upsertImportedClosedTrade(tx, {
            ...
            openedAt: this.dateOrNow(row.opening_time_utc ?? row.open_time),
            closedAt: this.dateOrNow(row.closing_time_utc ?? row.close_time),
          });
  ...
  private parseCsv(fileBuffer: Buffer): CsvTradeRow[] {
    try {
      ...
      throw new BadRequestException(
        'รูปแบบไฟล์ CSV ไม่ถูกต้อง หรือไฟล์เสียหาย',
      );
    }
  }

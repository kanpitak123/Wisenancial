...
export class WatchlistService {
   ...
   */
  async getWatchlist(userId: number, portfolioId: number) {
  ...
  async getUserWatchlist(
    ...
    return this.getByScope(userId, scope, currency);
  }
  ...
  async addToWatchlist(
    ...
    const metadata = await this.requireMatchingAsset(
      portfolio.portfolio_type,
      symbol,
    );
    ...
    if (existing) {
      throw new ConflictException(`${symbol} อยู่ใน Watchlist แล้ว`);
    }
    ...
    return this.prisma.watchlist.create({
      data: {
        ...
        market_region: metadata.marketRegion,
      },
    });
  }
  ...
  async removeFromWatchlist(
    ...
    if (
      !item ||
      !this.matchesPortfolioType(item.asset_type, portfolio.portfolio_type)
    ) {
  ...
  private async getByScope(
    ...
    const symbols = records.map((item) => item.symbol);
    const currency = String(currencyInput || 'USD')
      .trim()
      .toUpperCase();
    ...
    return records.map((record) => ({
      ...
      current_price: priceMap.get(record.symbol) ?? null,
    }));
  }
  ...
  private async requireMatchingAsset(type: PortfolioType, symbol: string) {
    if (type === PortfolioType.TRADER) {
      ...
      return {
        ...
        assetType: asset.asset_type.trim().toUpperCase(),
        marketRegion: 'GLOBAL',
      };
    }
    ...
    return {
      ...
      marketRegion:
        ...
        stock.country?.trim().toUpperCase() === 'THAILAND'
          ? 'TH'
          : 'GLOBAL',
    };
  }
  ...
  private matchesPortfolioType(
    ...
    return portfolioType === PortfolioType.INVESTOR
      ? assetType === 'STOCK'
      : assetType !== 'STOCK';
  }
  ...
  private normalizeSymbol(symbolInput: string): string {
    ...
    if (symbol.length > 50) {
      throw new BadRequestException('symbol ยาวเกินกำหนด');
    }

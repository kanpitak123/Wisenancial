...
import { PortfolioType, RecordStatus, RecordType } from '@prisma/client';
import { PrismaService } from '../prisma/prisma.service';
import { MarketService } from '../market/market.service';
import { RecordsService } from '../records/records.service';
import { StockPurchasesService } from '../stock-purchases/stock-purchases.service';
import type { AnalyticsTimeframe, PerformancePoint } from './analytics.types';
...
export class InvestorAnalyticsService {
  ...
  async overview(portfolioId: number, userId: number) {
    const portfolio = await this.assertPortfolio(portfolioId, userId);

    const [holdingRows, sales, dividends, recordSummary, recentActivity] =
      await Promise.all([
        ...
        this.records.getSummary(portfolioId, userId),
        this.records.findAll(portfolioId, userId, {
          limit: 20,
        }),
      ]);

    const investedCost = holdingRows.reduce(
      (sum, item) => sum + Number(item.cost_basis ?? 0),
      0,
    );
    ...
    const unrealizedPnl = holdingRows.reduce(
      (sum, item) => sum + Number(item.unrealized_pnl ?? 0),
      0,
    );

    const realizedPnl = Number(sales._sum.realized_pnl ?? 0);

    const dividendIncome = Number(dividends._sum.net_amount ?? 0);

    const cash = Number(portfolio.current_balance);
    const portfolioValue = cash + marketValue;

    const contributionTypes = new Set<RecordType>([
      RecordType.DEPOSIT,
      RecordType.WITHDRAW,
    ...
    const netContributions = Object.entries(recordSummary.totals).reduce(
      (sum, [type, row]) => {
        if (!contributionTypes.has(type as RecordType)) {
          return sum;
        }
    ...
    const contributedCapital =
      Number(portfolio.initial_balance) + netContributions;

    const investmentGain = portfolioValue - contributedCapital;
    ...
    return {
      ...
      summary: {
        ...
        holdings_value: marketValue,
        realized_pnl: realizedPnl,
        unrealized_pnl: unrealizedPnl,
        dividend_income: dividendIncome,
        total_pnl: realizedPnl + unrealizedPnl + dividendIncome,
        total_pnl_percent:
          contributedCapital > 0
            ? (investmentGain / contributedCapital) * 100
            : 0,
        contributed_capital: contributedCapital,
        investment_gain: investmentGain,
        open_holdings: holdingRows.length,
        closed_sales: sales._count,
      },
  ...
  async performance(
    ...
    const [records, purchases, sales] = await Promise.all([
      this.records.findAll(portfolioId, userId, { limit: 10_000 }),
    ...
    const symbols = [
      ...new Set([
        ...purchases.map((row) => row.stock_symbol.toUpperCase()),
        ...sales.map((row) => row.stock_symbol.toUpperCase()),
      ]),
    ];

    const prices: Record<string, number | null> =
      symbols.length > 0 ? await this.market.getQuotes(symbols) : {};
    ...
    for (const symbol of symbols) {
      const lots = purchases.filter(
        (row) => row.stock_symbol.toUpperCase() === symbol,
      );
      const totalShares = lots.reduce(
        (sum, row) => sum + Number(row.shares_count),
        0,
      );
      ...
      if (totalShares > 0) {
        averageCost.set(symbol, totalCost / totalShares);
      }
    }

    const priceOf = (symbol: string) =>
      prices[symbol] ?? averageCost.get(symbol) ?? 0;
    ...
    const events: Event[] = [
      ...records.map((record) => ({
        at: new Date(record.occurred_at),
        cashDelta: Number(record.amount),
        label: String(record.type),
        amount: Number(record.amount),
      })),
      // การซื้อ/ขายกระทบเงินสดผ่าน records อยู่แล้ว ที่นี่จึงนับเฉพาะจำนวนหุ้น
      ...purchases.map((row) => ({
        at: new Date(row.purchase_date),
        cashDelta: 0,
        symbol: row.stock_symbol.toUpperCase(),
        sharesDelta: Number(row.shares_count),
        label: 'BUY_SHARES',
      })),
      ...sales.map((row) => ({
        ...
        cashDelta: 0,
        symbol: row.stock_symbol.toUpperCase(),
        sharesDelta: -Number(row.shares_sold),
        label: 'SELL_SHARES',
      })),
    ].sort((a, b) => a.at.getTime() - b.at.getTime());

    let cash = Number(portfolio.initial_balance);
    ...
    const emit = (date: string | Date, label: string, amount?: number) => {
      points.push({
        date,
        value: this.round(cash + holdingsValue()),
        event: label,
        ...(amount !== undefined
          ? {
    ...
    for (const event of events) {
      ...
      if (start && event.at < start) {
        cash += event.cashDelta;

        if (event.symbol && event.sharesDelta) {
          shares.set(
            event.symbol,
            (shares.get(event.symbol) ?? 0) + event.sharesDelta,
          );
        }
      ...
      if (points.length === 0) {
        emit(start?.toISOString() ?? 'START', 'START');
      }
      ...
      emit(event.at, event.label, event.amount);
    }
  ...
  async timeline(
    ...
    return this.records.findAll(portfolioId, userId, {
      limit: 500,
  ...
  async allocation(portfolioId: number, userId: number) {
    await this.assertPortfolio(portfolioId, userId);

    const holdings = await this.holdings.getHoldings(portfolioId, userId);

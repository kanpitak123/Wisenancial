...
import {
  CreateInvestorRecordDto,
  ManualRecordType,
} from './dto/create-investor-record.dto';
      ...
      async (tx) => {
        ...
        if (updatedPortfolio.current_balance.isNegative()) {
          throw new BadRequestException(
            'ยอดเงินคงเหลือไม่เพียงพอสำหรับรายการนี้',
          );
        }
      ...
      async (tx) => {
        ...
        const record = await tx.records.update({
          ...
          data: {
            ...
            ...(dto.record_date !== undefined && {
              occurred_at: new Date(dto.record_date),
            }),
          },
        });
  ...
  async remove(id: number, userId: number) {
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (portfolio.current_balance.isNegative()) {
          throw new BadRequestException(
            'ไม่สามารถยกเลิกรายการนี้ เพราะจะทำให้ยอดเงินคงเหลือติดลบ',
          );
        }
  ...
  private async findManualRecord(id: number, userId: number) {
    ...
    if (!record) {
      throw new NotFoundException(
        'ไม่พบรายการ หรือรายการนี้เป็นรายการระบบที่แก้ไขไม่ได้',
      );
    }
    return record;
  }

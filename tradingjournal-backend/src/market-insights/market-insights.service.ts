...
export class MarketInsightsService {
  ...
  getSentiment(market: HeatmapMarket = 'GLOBAL'): SentimentResponse {
    const seed =
      market === 'TH' ? this.sentimentThSeed : this.sentimentGlobalSeed;
    ...
    return {
      ...
      overall: {
        longPercent: overallLong,
        shortPercent: Number((100 - overallLong).toFixed(1)),
      },
      longShortRatios: longShortRatios.sort(
        (a, b) => b.longPercent - a.longPercent,
      ),
      mostBought,
      mostSold,
      frequentSetups: [...this.setupSeed].sort(
